data {
  ...
  vector[sum(T)] x;                   // Reverse-scored item indicators
  matrix[N, sum(T)] r;                // Item responses
  real ars_prior;                     // SD-Prior for ARS
}
...
parameters {
  array[N] vector[theta_n] theta; // Latent traits
  vector[N] ars;                      // ARS
  vector[K] delta;                    // Item difficulties
  vector<lower=0.1>[K] tau;           // Slightly relaxed lower bound
  vector<lower=0>[theta_n] sigma; // Standard deviations
  corr_matrix[theta_n] Sigma_corr;// Correlation matrix
}
...
transformed parameters {
  ...
  matrix[theta_n, theta_n] L_Sigma = diag_pre_multiply(sigma, cholesky_decompose(Sigma_corr));    // Cholesky factor of the covariance matrix for theta(s)

  {
    ...
    for (i in 1:theta_n) {
      for (k in k_lower:k_upper) {
        ...
        for (j in 1:N)        
        core[j] = (theta[j, i] - delta[k] + x[k] * ars[j]); // compute the core
...
model {
  ...
  ars ~ normal(0, ars_prior);        // prior depends on the simulation condition
  sigma ~ normal(0, 1);              // standard normal prior for SD
  
  // Vectorized and Cholesky factorized latent traits
  theta ~ multi_normal_cholesky(rep_vector(0, theta_n), L_Sigma);  // multivariate prior for theta(s) 
